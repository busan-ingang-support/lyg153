...
courseQna.post('/', async (c) => {
  ...
  try {
    ...
    const result = await db.prepare(`
      INSERT INTO course_qna (site_id, course_id, student_id, title, question, is_private, status)
      VALUES (?, ?, ?, ?, ?, ?, 'pending')
    `).bind(siteId, course_id, student.id, title, question, is_private || 0).run();
...
courseQna.post('/:id/answer', async (c) => {
  ...
  try {
    ...
    `).bind(answer, teacher.id, id, siteId).run();
