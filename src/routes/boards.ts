...
boards.get('/', async (c) => {
  ...
  try {
    let query = 'SELECT * FROM boards WHERE is_active = 1';
    const params: any[] = [];
...
boards.get('/posts/:id', async (c) => {
  ...
  try {
    // 조회수 증가
    await db.prepare('UPDATE board_posts SET view_count = view_count + 1 WHERE id = ?').bind(id).run();
    ...
    const { results: comments } = await db.prepare(`
      ...
      WHERE bc.post_id = ? AND bc.is_deleted = 0
...
boards.post('/posts', async (c) => {
  ...
  try {
    ...
    const result = await db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `).bind(board_id, author_id, title, content, is_notice || 0).run();
...
boards.post('/comments', async (c) => {
  ...
  try {
    ...
    const result = await db.prepare(`
      ...
      VALUES (?, ?, ?, ?)
    `).bind(post_id, author_id, content, parent_comment_id || null).run();
